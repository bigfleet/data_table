...
data_table is slated for its first use in production any day now. If you feel
capable of finding and fixing your own bugs, data_table should be ready for
you. It's already meeting some advanced use-cases, so I hope it's of immediate
use.
...
The full picture of data_table has come into view. * optional filtering *
optional sorting * included pagination

At its simplest, data_table can be a wrapper around will_paginate. At it's
most complex, it can ensure that information is supplied only once, and shared
and used among those interrelated pieces as freely as possible
...
that you can do, check out these wishlists. I have not yet updated them to be
in line with the codebase, but it should give you a lot better idea about the
API that I am aiming for.
